...
# class C IP version
cat $TDIR"temp.txt"  | cut -d ' ' -f 9 | sed -e 's/SRC=//g' | sed -e 's/DPT=//g' | sed -e 's/ /:/g' |cut -d '.' -f 1,2,3 > $TDIR"temp2b.txt"
...
echo "-- Number of unique Class C IP attacking   : $UNIQUE_IPS"
echo "   Average # of attacks per Class C IP     : $ATKPERIP"
...
echo "Attacks:  Class C IP:  Country:"
echo "-------------------------------"
IFS=''
...
#        COUNTRY_LINE=`whois $IP | tac | grep -i -m 1 'country:'`
        COUNTRY_LINE=`whois $IP.1 | tac | grep -i -m 1 'country:'`
